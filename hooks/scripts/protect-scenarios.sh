...
# /balka:spec, has to write .feature files itself, so an unconditional block
...
balka_deny "Blocked: $rel is an existing scenario file.
...
Routes through:
  - The contract is genuinely wrong, or the behaviour changed: run
    /balka:spec. To replace a scenario there, \`git rm\` the file and write a
